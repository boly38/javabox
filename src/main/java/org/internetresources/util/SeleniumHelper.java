 ...
 * http://stackoverflow.com/questions/20153104/htmlunit-
 * not-working-with-angularjs using <html id="ng-app"
 * class="ng-app: appmodule;"> instead of <html ng-app="appmodule">
 * 
 * TIP wait handlers example : src:
...
public class SeleniumHelper {
    ...
    public static void usageExample() {
        ...
        driver.manage().timeouts()
                .implicitlyWait(MAX_WAIT_SEC, TimeUnit.SECONDS);
        ...
        loginButton.click();
        seleniumHelper.waitUntilTextIsPresent("loginNotificationDiv",
    ...
    public void waitUntilTextIsPresent(String elementId, String txtMsg) {
        waitDriver.ignoring(NoSuchElementException.class)
                .ignoring(StaleElementReferenceException.class)
                .pollingEvery(2, TimeUnit.SECONDS)
                .until(textIsPresent(elementId, txtMsg));
    }

    protected Function<WebDriver, Boolean> textIsPresent(String elementId,
            String text) {
        ...
        return new Function<WebDriver, Boolean>() {
            public Boolean apply(WebDriver driver) {
                ...
                boolean isElementContainerDisplayed = (elementContainer != null ? elementContainer
                        .isDisplayed() : false);
                String elementContainerText = (elementContainer != null ? elementContainer
                        .getText() : null);
    ...
    public void waitUntilClassIsPresent(String elementId, String classToBe,
            int customTimeoutSec, int customPollingIntervalSec) {
        WebDriverWait customWaitDriver = new WebDriverWait(driver,
                customTimeoutSec);
        customWaitDriver.ignoring(NoSuchElementException.class)
                .ignoring(StaleElementReferenceException.class)
    ...
    protected Function<WebDriver, Boolean> classIsPresent(String elementId,
            String classToBe) {
        final String elemId = elementId;
    ...
    private boolean isClassPresent(final String elemId, final String clzz,
            WebElement elementContainer) {
        String[] elemClzzs = (elementContainer != null && elementContainer
                .getAttribute("class") != null) ? elementContainer
                .getAttribute("class").split(" ") : null;
    ...
    public void assertInputField(String inputFieldId, String expectedValue) {
        ...
        if (!expectedValue.equals(fieldCurValue)) {
            String assertInputError = "#" + inputFieldId + " value="
                    + fieldCurValue + " expected=" + expectedValue;
            LOG.error(assertInputError);
            throw new AssertionError(assertInputError);
        }
    }

    public void assertClassPresent(String elementId, String expectedClassId) {
        WebElement targetElement = driver.findElement(By.id(elementId));
        boolean classPresent = isClassPresent(elementId, expectedClassId,
                targetElement);
        if (!classPresent) {
            String assertInputError = "#" + elementId + " class="
    ...
    public Cookie getCookie(String cName) {
        Cookie cookieValue = driver.manage().getCookieNamed(cName);
        LOG.debug("#cookie#" + cName + "="
                + (cookieValue != null ? cookieValue.getValue() : "(null)"));
        if (cookieValue != null) {
            return cookieValue;
        }
        Set<Cookie> allCookies = driver.manage().getCookies();
        if (allCookies == null || allCookies.size() < 1) {
            return cookieValue;
        }
        List<String> cookNames = new ArrayList<String>(allCookies.size());
        for (Cookie c : allCookies) {
            cookNames.add(c.getName());
        }
        LOG.debug("unable to find cookie '" + cName + "' into "
                + Arrays.toString(cookNames.toArray()));
        return cookieValue;
    }
}
